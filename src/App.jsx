...
export default function App() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setHeading((h) => (h + (Math.random() - 0.48) * 1.5 + 360) % 360);
      setDepth((d) => Math.max(0, Math.min(180, d + (Math.random() - 0.5) * 0.3)));
      setBattery((b) => Math.max(0, b - 0.01));
      setSpeed((s) => Math.max(0, Math.min(5, s + (Math.random() - 0.5) * 0.1)));
    }, 500);
    ...
    <div className="app">
      ...
      <header className="topbar">
        ...
        <div className="topbar-status">
          <div className={`status-dot ${mode === 'auto' ? 'auto' : 'manual'}`} />
          <span className="status-text">{mode === 'auto' ? 'AUTONOMOUS' : 'MANUAL CONTROL'}</span>
      ...
      <div className="main-layout">
        ...
        <aside className="sidebar-left">
          ...
          <div className="stat-block">
            ...
            <div className="stat-value">{speed.toFixed(1)} <span className="stat-unit">kn</span></div>
            <div className="speed-bar">
              <div className="speed-fill" style={{ width: `${(speed / 5) * 100}%` }} />
            </div>
          </div>
          <div className="divider" />
          {/* Mode toggle */}
        ...
        <main className="center-panel">
          <div className="map-header">
            ...
            <div className="map-actions">
              {waypoints.length > 0 && (
                <span className="waypoint-count">{waypoints.length} waypoint{waypoints.length !== 1 ? 's' : ''}</span>
              )}
              {waypoints.length > 0 && (
                <button className="clear-btn" onClick={clearWaypoints}>CLEAR TRAIL</button>
              )}
            </div>
          </div>
          <DroneMap
